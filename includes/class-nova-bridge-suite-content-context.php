<?php
/**
 * Content endpoint discovery and user-defined REST field guidance.
 *
...
final class Nova_Bridge_Suite_Content_Context {
	...
	/**
	 * Adds the always-on runtime hooks.
	 *
	 * The host plugin is responsible for loading this file and calling bootstrap()
	 * before its request-specific early-return logic.
	 */
	...
	/**
	 * Enqueues the optional settings application from the plugin root assets folder.
	 */
	public static function enqueue_admin_assets( string $hook ): void {
		...
		$plugin_dir = defined( 'NOVA_BRIDGE_SUITE_PLUGIN_DIR' ) ? NOVA_BRIDGE_SUITE_PLUGIN_DIR : trailingslashit( dirname( __DIR__ ) );
		$plugin_url = defined( 'NOVA_BRIDGE_SUITE_PLUGIN_FILE' )
			? plugin_dir_url( NOVA_BRIDGE_SUITE_PLUGIN_FILE )
			: plugin_dir_url( dirname( __FILE__ ) );
		$version    = defined( 'NOVA_BRIDGE_SUITE_VERSION' ) ? NOVA_BRIDGE_SUITE_VERSION : self::SCHEMA_VERSION;
		$script     = $plugin_dir . 'assets/content-context-admin.js';
		$style      = $plugin_dir . 'assets/content-context-admin.css';

		if ( file_exists( $style ) ) {
			wp_enqueue_style(
				'nova-bridge-suite-content-context',
				$plugin_url . 'assets/content-context-admin.css',
				[],
				(string) $version
			);
		}
		...
		wp_enqueue_script(
			'nova-bridge-suite-content-context',
			$plugin_url . 'assets/content-context-admin.js',
			[],
			(string) $version,
			true
		);
	...
	public static function render_settings_tab(): void {
		...
		<div class="nova-content-context-settings">
			...
			<form method="post" action="options.php" id="nova-content-context-form">
				...
				<?php submit_button( __( 'Save Content Context', 'nova-bridge-suite' ) ); ?>
			</form>
		</div>
		<?php
	}
