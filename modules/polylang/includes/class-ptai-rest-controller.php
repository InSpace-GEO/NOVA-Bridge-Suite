...
class PTAI_REST_Controller extends \WP_REST_Controller
{
    private PTAI_Translation_Service $translation_service;

    public function __construct(PTAI_Translation_Service $translation_service)
    {
        $this->namespace           = 'polylang-translations/v1';
    ...
    public function create_translations(\WP_REST_Request $request)
    {
        ...
        if ($source_post_id <= 0) {
            return new \WP_Error('ptai_missing_source', 'source_post_id is required.', ['status' => 400]);
        }

        if (! is_array($translations) || empty($translations)) {
            return new \WP_Error('ptai_missing_translations', 'translations array is required.', ['status' => 400]);
        }
    ...
    public function create_term_translations(\WP_REST_Request $request)
    {
        ...
        $taxonomy       = $request->get_param('taxonomy');
        $translations   = $request->get_param('translations');
        $group_hint     = (int) $request->get_param('trid');

        if ($source_term_id <= 0) {
            return new \WP_Error('ptai_missing_source_term', 'source_term_id is required.', ['status' => 400]);
        }

        if (empty($taxonomy) || ! is_string($taxonomy)) {
            return new \WP_Error('ptai_missing_taxonomy', 'taxonomy is required.', ['status' => 400]);
        }
        ...
        foreach ($translations as $translation) {
            ...
            $result = $this->translation_service->upsert_term_translation($source_term_id, $taxonomy, $translation, $group_hint ?: null);
    ...
    public function get_item_schema(): array
    {
        return [
            ...
            'title'      => 'polylang_post_translations',
            'type'       => 'object',
            'properties' => [
                'source_post_id' => [
                ...
                'translations'  => [
                    ...
                    'items'       => [
                        ...
                        'properties' => [
                            ...
                            'post_id'        => [
                                'description' => 'Optional existing translated post ID for update.',
                                'type'        => 'integer',
                            ],
                            'translation_id' => [
    ...
    private function get_term_endpoint_args(): array
    {
        return [
            ...
            'trid'           => [
                'description' => 'Optional legacy group hint (accepted for WPML-flow compatibility, ignored by Polylang bridge).',
                'type'        => 'integer',
                'required'    => false,
                'minimum'     => 1,
            ],
