<?php
/**
 * NOVA API Mapping Context discovery and REST privacy canary.
 *
...
try {
	...
	nova_content_context_test_assert(
		'modules/api-mapping-context/api-mapping-context.php' === ( $module_definition['path'] ?? '' ),
		'API Mapping Context has its own module definition outside the CPT modules.'
	);
	nova_content_context_test_assert(
		1 === ( nova_bridge_suite_default_settings()['api_mapping_context'] ?? 0 ),
		'API Mapping Context defaults to enabled so existing mapping installations keep working.'
	);
	$module_root = dirname( __DIR__ );
	$class_file  = ( new ReflectionClass( 'Nova_Bridge_Suite_Content_Context' ) )->getFileName();
	nova_content_context_test_assert(
		is_string( $class_file )
		&& wp_normalize_path( $module_root . '/includes/class-nova-bridge-suite-content-context.php' ) === wp_normalize_path( $class_file )
		&& file_exists( $module_root . '/api-mapping-context.php' )
		&& file_exists( $module_root . '/assets/content-context-admin.js' )
		&& file_exists( $module_root . '/assets/content-context-admin.css' )
		&& file_exists( NOVA_BRIDGE_SUITE_PLUGIN_DIR . 'includes/class-nova-bridge-suite-content-context.php' ),
		'The runtime and assets are module-owned while the legacy class path remains available as a forwarding shim.'
	);
	nova_content_context_test_assert(
		'nova_bridge_suite_content_contexts' === Nova_Bridge_Suite_Content_Context::OPTION_NAME,
		'The extraction preserves the existing saved mapping option name.'
	);

	$mapping_option_before_module_toggle = get_option( $option_name, false );
	$legacy_suite_settings = is_array( $old_suite_settings ) ? $old_suite_settings : [];
	unset( $legacy_suite_settings['api_mapping_context'] );
	update_option( NOVA_BRIDGE_SUITE_OPTION, $legacy_suite_settings, false );
	nova_content_context_test_assert(
		! empty( nova_bridge_suite_get_settings()['api_mapping_context'] ),
		'A legacy settings record with no module key resolves API Mapping Context as enabled.'
	);
	$explicitly_disabled_settings                        = $legacy_suite_settings;
	$explicitly_disabled_settings['api_mapping_context'] = 0;
	update_option( NOVA_BRIDGE_SUITE_OPTION, $explicitly_disabled_settings, false );
	nova_content_context_test_assert(
		empty( nova_bridge_suite_get_settings()['api_mapping_context'] ),
		'An explicitly saved API Mapping Context value of zero remains disabled.'
	);
	nova_content_context_test_assert(
		$mapping_option_before_module_toggle === get_option( $option_name, false ),
		'Toggling the module setting does not delete or rewrite saved mappings.'
	);
	...
	nova_bridge_suite_register_settings();
	Nova_Bridge_Suite_Content_Context::register_settings();
	global $wp_settings_fields;
	nova_content_context_test_assert(
		isset( $wp_settings_fields['nova-settings']['nova_bridge_api_mapping']['nova_bridge_api_mapping_context_toggle'] )
		&& ! isset( $wp_settings_fields['nova-settings']['nova_bridge_cpt']['nova_bridge_api_mapping_context_toggle'] ),
		'The module toggle lives in its own API Mapping Context settings section, not Custom Post Types.'
	);
	...
	nova_content_context_test_assert(
		...
		&& $context_script
		&& false !== strpos( (string) $context_script->src, '/modules/api-mapping-context/assets/content-context-admin.js' )
		&& wp_style_is( 'nova-bridge-suite-content-context', 'enqueued' )
		&& $context_style
	...
	// register its bridge field and standard controller, then refresh NOVA's late
	// hooks. This mirrors the normal rest_api_init order for the temporary type.
	if ( $rest_init_at_start > 0 ) {
		global $wp_rest_additional_fields;
		if (
			function_exists( 'cf_tmrb_get_post_meta_all_payload' )
			&& function_exists( 'cf_tmrb_update_post_meta_all_payload' )
			&& empty( $wp_rest_additional_fields[ $visible_post_type ]['meta_all'] )
		) {
			register_rest_field(
				$visible_post_type,
				'meta_all',
				[
					'get_callback'    => static function ( $post_array, $field_name = null, $request = null ) {
						$post_id = absint( $post_array['id'] ?? 0 );
						if ( function_exists( 'cf_tmrb_should_expand_rest_meta_field' ) && ! cf_tmrb_should_expand_rest_meta_field( $request ) ) {
							return [];
						}

						return cf_tmrb_get_post_meta_all_payload( $post_id, current_user_can( 'edit_post', $post_id ) );
					},
					'update_callback' => 'cf_tmrb_update_post_meta_all_payload',
					'schema'          => [
						'description'          => 'All post meta (ACF-aware).',
						'type'                 => 'object',
						'context'              => [ 'view', 'edit' ],
	...
	nova_content_context_test_assert(
		isset( $registered_routes[ $discovery_rest_route ], $registered_routes[ $bridge_fields_route ] ),
		'The enabled module registers both API Mapping Context REST routes.'
	);
